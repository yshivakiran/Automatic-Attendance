...
window = tk.Tk()
window.title("Automatic Attendance and Monitoring System")
...
from tkinter import simpledialog, messagebox  # Ensure these imports are at the top of your code

def merge_files_by_prefix():
    folder_path = r"C:\Users\91756\Desktop\FaceRec-Attendance\FaceRec-Attendance\Attendance"  # Update this path if needed
    file_prefix = simpledialog.askstring("Input", "Enter the prefix for the files to merge:")
    
    if file_prefix is None:  # User cancelled the input
        return
    ...
    for filename in os.listdir(folder_path):
        if filename.startswith(file_prefix) and filename.endswith(".csv"):
            ...
            merged_data = pd.concat([merged_data, data], ignore_index=True)  # Merge data

    # Check if merged_data is not empty
    if not merged_data.empty:
        merged_data.to_csv(f"C:\\Users\\91756\\Desktop\\FaceRec-Attendance\\FaceRec-Attendance\\merged\\{file_prefix}_merged.csv", index=False)  # Save merged data to a new CSV
        messagebox.showinfo("Success", f"Merged data saved as {file_prefix}_merged.csv")
    else:
        messagebox.showwarning("No Files Found", f"No files found with the prefix '{file_prefix}'.")
...
def manually_fill():
    ...
    sb.geometry('580x320')
    sb.configure(background='AliceBlue')
    sb.eval('tk::PlaceWindow . center')
    ...
    def fill_attendance():
        ...
        if subb == '':
            ...
            MFW.configure(background='AliceBlue')
            ...
            def err_screen1():
                ...
                errsc2.configure(background='AliceBlue')
                Label(errsc2, text='Please enter Student & Enrollment!!!', fg='black', bg='white', font=('times', 16, ' bold ')).pack()
                Button(errsc2, text='OK', command=del_errsc2, fg="black", bg="lawn green", width=9, height=1, activebackground="Red", font=('times', 15, ' bold ')).place(x=90, y=50)
            ...
            c1ear_enroll = tk.Button(MFW, text="Clear", command=remove_enr, fg="white", bg="black", width=10,height=1,activebackground="white", font=('times', 15, ' bold '))
            c1ear_enroll.place(x=690, y=100)

            c1ear_student = tk.Button(MFW, text="Clear", command=remove_student, fg="white", bg="black", width=10,height=1,activebackground="white", font=('times', 15, ' bold '))
            c1ear_student.place(x=690, y=200)

            DATA_SUB = tk.Button(MFW, text="Enter Data", command=enter_data_DB, fg="black", bg="SkyBlue1", width=20,height=2,activebackground="white", font=('times', 15, ' bold '))
            DATA_SUB.place(x=170, y=300)

            MAKE_CSV = tk.Button(MFW, text="Convert to CSV", command=create_csv, fg="black", bg="SkyBlue1", width=20,height=2,activebackground="white", font=('times', 15, ' bold '))
            MAKE_CSV.place(x=570, y=300)

            # def attf():
            #     import subprocess
            #     subprocess.Popen(r'explorer /select,"Attendance/Manually Attendance"')
    ...
    SUB = tk.Label(sb, text="Enter Subject : ", width=15, height=2, fg="black", bg="AliceBlue", font=('times', 15, ' bold '))
    ...
    fill_manual_attendance = tk.Button(sb, text="Fill Attendance", command=fill_attendance, fg="black", bg="SkyBlue1", width=20, height=2,activebackground="white", font=('times', 15, ' bold '))
...
def err_screen():
    ...
    sc1.title('Warning!!')
    sc1.configure(background='AliceBlue')
    sc1.eval('tk::PlaceWindow . center')
    tk.Label(sc1, text='Enrollment & Name required!!!', fg='black',bg='white', font=('times', 16)).pack()
...
def err_screen1():
    ...
    sc2.title('Warning!!')
    sc2.configure(background='AliceBlue')
    tk.Label(sc2, text='Please enter your subject name!!!', fg='black',bg='white', font=('times', 16)).pack()
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                attendance = attendance.drop_duplicates(['Enrollment'], keep='last')
                print(attendance)
                attendance.to_csv(fileName, index=False)
                # df['total presents']=df.apply(lambda row: (row =='').sum(),axis=1)
                # total_present =df['total presents'].sum()
                # print(f"total present:{total_present}")
                # df.to_excel("attendance_with_total.csv",index=False)
    ...
    windo.configure(background='AliceBlue')
    ...
    def Attf():
        import subprocess
        subprocess.Popen(r'explorer /select,"C:/Users/91756/Desktop/New folder/FaceRec-Attendance/Attendance"')
    ...
    fill_a = tk.Button(windo, text="Fill Attendance", fg="white", command=Fillattendances, bg="SkyBlue1", width=20, height=2,activebackground="white", font=('times', 15, ' bold '))
    fill_a.place(x=250, y=160)
    windo.mainloop()
    
# Adding total number of students present at the end of the Excel sheet
def add_total_students_present(file_name):
    try:
        # Load the existing attendance data
        df = pd.read_csv(file_name)
        
        # Calculate the total number of students marked as present
        total_present = len(df)

        # Append the total count as a new row at the end
        total_row = pd.DataFrame([{'Name': 'Total Students Present', 'Roll': '', 'Date': '', 'Time': '', 'Status': total_present}])
        df = pd.concat([df, total_row], ignore_index=True)

        # Save back to the file
        df.to_csv(file_name, index=False)
        print(f"Total Students Present count added to {file_name}.")
    except Exception as e:
        print(f"Error adding total students present: {e}")
...
def admin_panel():
    ...
    win.configure(background='AliceBlue')
    ...
    def log_in():
        ...
        if username == 'test':
            if password == '1234':
                ...
                Nt.configure(text=valid, bg="red", fg="white",width=38, font=('times', 19, 'bold'))
    ...
    c0 = tk.Button(win, text="Clear", command=c00, fg="white", bg="black", width=10, height=1,activebackground="white", font=('times', 15, ' bold '))
    c0.place(x=690, y=55)

    c1 = tk.Button(win, text="Clear", command=c11, fg="white", bg="black", width=10, height=1,activebackground="white", font=('times', 15, ' bold '))
    c1.place(x=690, y=155)

    Login = tk.Button(win, text="LogIn", fg="black", bg="SkyBlue1", width=20,height=2,activebackground="Red", command=log_in, font=('times', 15, ' bold '))
...
message = tk.Label(window, text="Automatic Attendance and Monitoring System", bg="black", fg="white", width=50,height=3, font=('times', 30, ' bold '))
...
AP = tk.Button(window, text="Check Registered students", command=admin_panel, fg="black",bg="SkyBlue1", width=19, height=1, activebackground="white", font=('times', 15, ' bold '))
AP.place(x=990, y=410)

takeImg = tk.Button(window, text="Take Images", command=take_img, fg="black", bg="SkyBlue1",width=20, height=3, activebackground="white", font=('times', 15, ' bold '))
takeImg.place(x=90, y=500)

trainImg = tk.Button(window, text="Train Images", fg="black", command=trainimg, bg="SkyBlue1",width=20, height=3, activebackground="white", font=('times', 15, ' bold '))
trainImg.place(x=390, y=500)

FA = tk.Button(window, text="Automatic Attendance", fg="black", command=subjectchoose,bg="SkyBlue1", width=20, height=3, activebackground="white", font=('times', 15, ' bold '))
FA.place(x=690, y=500)

quitWindow = tk.Button(window, text="Manually Fill Attendance", command=manually_fill, fg="black",bg="SkyBlue1", width=20, height=3, activebackground="white", font=('times', 15, ' bold '))
quitWindow.place(x=990, y=500)

# Add a button to the main window to open the merge files dialog
merge_button = tk.Button(window, text="Merge CSV Files", command=merge_files_by_prefix, fg="black", bg="SkyBlue1", width=20, height=3, activebackground="white", font=('times', 15, ' bold '))
merge_button.place(x=990, y=600)  # Adjust the position as needed

from tkinter import messagebox, simpledialog

# # Function to merge CSV files based on a given prefix
# def merge_files_by_prefix():
#     folder_path = r"C:\Users\91756\Desktop\FaceRec-Attendance\FaceRec-Attendance\Attendance"  # Update this path
#     file_prefix = entry.get()  # Get the input from the entry widget
#     merged_data = pd.DataFrame()  # Initialize an empty DataFrame
...
# # Create a label and entry for file prefix
# label = tk.Label(root, text="Enter file prefix:")
# label.pack(pady=10)
...
# merge_button = tk.Button(root, text="Merge CSV Files", command=merge_files_by_prefix)
